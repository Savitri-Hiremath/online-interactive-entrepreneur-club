...
foreach($posts as $post){
    ...
    ?>
     <div class="card mt-4" height="100" width="100" >
                <div class="card-title d-flex justify-content-between  align-items-center" height="100" width="100">

                    <div class="d-flex align-items-center p-2">
                        <img src="assets/images/profile/<?=$post['profilepic']?>" alt="" height="30" width="30" class="rounded-circle border">&nbsp;&nbsp;<a href='?u=<?=$post['username']?>' class="text-decoration-none text-dark"><?=$post['firstname']?> <?=$post['lastname']?></a>
                    </div>
                    <div class="p-2">
                        <?php
if($post['uid']==$user['id']){
  ...
  <ul class="dropdown-menu" aria-labelledby="option<?=$post['id']?>">
    <li><a class="dropdown-item" href="assets/php/process.php?deletepost=<?=$post['id']?>"><i class="bi bi-trash-fill"></i> Delete Post</a></li>
  </ul>
</div>
    <?php
}
                ...
                <img src="assets/images/posts/<?=$post['post_img']?>" loading=lazy class="" alt="..." height="400" width="700">
                <?php
        if ($post['post_text']) {
            ?>
            <div class="card-body">
                <p><strong><a href="?u=<?=$post['username']?>" class="text-decoration-none text-dark">@<?=$post['username']?></a>:</strong> <?=$post['post_text']?></p>
            </div>
            <?php
        }
...
$unlike_btn_display='';
                ...
                <div class="modal-body d-md-flex p-0">
                    ...
                    <div class="col-md-4 col-sm-12 d-flex flex-column">
                        <div class="d-flex align-items-center p-2 border-bottom">
                            <div><img src="assets/images/profile/<?=$post['profilepic']?>" alt="" height="50" width="50" class="rounded-circle border">
                            </div>
                            <div>&nbsp;&nbsp;&nbsp;</div>
                            <div class="d-flex flex-column justify-content-start">
                                <h6 style="margin: 0px;"><?=$post['firstname']?> <?=$post['lastname']?></h6>
  ...
  foreach($likes as $like){
      ...
      ?>
  <li><a class="dropdown-item" href="?u=<?=$lu['username']?>"><?=$lu['firstname'].' '.$lu['lastname']?> (@<?=$lu['username']?>)</a></li>
...
foreach($comments as $comment){
    ...
    ?>
<div class="d-flex align-items-center p-2">
                                <div><img src="assets/images/profile/<?=$cuser['profilepic']?>" alt="" height="40" width="40" class="rounded-circle border">
                                </div>
                                <div>&nbsp;&nbsp;&nbsp;</div>
    ...
    <?php
                        ...
                        <div class="comments-container" style="display: none;" data-post-id="<?= $post['id'] ?>">
        <?php foreach ($comments as $comment) { ?>
            <!-- Your existing comment code ... -->

            <!-- Reply input field and button -->
            <div class="comment-reply-container">
                <input type="text" class="form-control rounded-0 border-0 reply-input" placeholder="Reply..."
                    aria-label="Reply input" aria-describedby="button-addon2">
                <button class="btn btn-outline-primary rounded-0 border-0 reply-button" data-cs="comment-section<?= $post['id'] ?>"
                    data-comment-id="<?= $comment['id'] ?>" type="button" id="button-addon2">Reply</button>
            </div>
    ...
    <!-- Update the "View Comments" part -->
    <i class="bi bi-chat-left d-flex align-items-center">
        <span class="p-1 mx-2 text-small" style="font-size: small" onclick="toggleComments('<?= $post['id'] ?>')">
...
<div class="d-flex justify-content-between">
                    <div class="d-flex align-items-center p-2">
                        <div><img src="assets/images/profile/<?=$fuser['profilepic']?>" alt="" height="40" width="40" class="rounded-circle border">
                        </div>
                        <div>&nbsp;&nbsp;</div>
                        <div class="d-flex flex-column justify-content-center">
                            <a href='?u=<?=$fuser['username']?>' class="text-decoration-none text-dark"><h6 style="margin: 0px;font-size: small;"><?=$fuser['firstname']?> <?=$fuser['lastname']?></h6></a>
    ...
    <?php
        ...
        <div class="col-lg-4 col-sm-0 overflow-hidden mt-4 p-sm-0 p-md-3">
            ...
            <div class="d-flex align-items-center p-2">
                <div><img src="assets/images/profile/<?=$user['profilepic']?>" alt="" height="60" width="60" class="rounded-circle border">
                </div>
                <div>&nbsp;&nbsp;&nbsp;</div>
                <div class="d-flex flex-column justify-content-center">
                <a href='?u=<?=$user['username']?>' class="text-decoration-none text-dark"><h6 style="margin: 0px;"><?=$user['firstname']?> <?=$user['lastname']?></h6></a>
...
<div class="d-flex justify-content-between">
                    <div class="d-flex align-items-center p-2">
                        <div><img src="assets/images/profile/<?=$suser['profilepic']?>" alt="" height="40" width="40" class="rounded-circle border">
                        </div>
                        <div>&nbsp;&nbsp;</div>
                        <div class="d-flex flex-column justify-content-center">
                            <a href='?u=<?=$suser['username']?>' class="text-decoration-none text-dark"><h6 style="margin: 0px;font-size: small;"><?=$suser['firstname']?> <?=$suser['lastname']?></h6></a>
