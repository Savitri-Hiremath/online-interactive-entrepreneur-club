...
function validateSignupForm($form_data) {
    ...
    } elseif (!isPasswordValid($form_data['password'])) {
        // Add password conditions
        $response['msg'] = "Password must contain minimum 8 characters, Uppercase letters, Lowercase letters and any special character";
        $response['status'] = false;
        $response['field'] = 'password';
    }
...
function isPasswordValid($password) {
    ...
    // For example, minimum length, uppercase, lowercase, and special characters
    $minLength = 8;
    $hasUppercase = preg_match('/[A-Z]/', $password);
    $hasLowercase = preg_match('/[a-z]/', $password);
    $hasSpecialChar = preg_match('/[^a-zA-Z0-9]/', $password);

    return strlen($password) >= $minLength && $hasUppercase && $hasLowercase && $hasSpecialChar;
}
...
function searchUser($keyword){
    global $db;
 $query = "SELECT * FROM users WHERE username LIKE '%".$keyword."%' || (firstname LIKE '%".$keyword."%' || lastname LIKE '%".$keyword."%') LIMIT 5";
 $run = mysqli_query($db,$query);
 return mysqli_fetch_all($run,true);
...
function getPost(){
    global $db;
 $query = "SELECT users.id as uid,posts.id,posts.user_id,posts.post_img,posts.post_text,posts.created_at,users.firstname,users.lastname,users.username,users.profilepic FROM posts JOIN users ON users.id=posts.user_id ORDER BY id DESC";
...
function verifyEmail($email){
    global $db;
    $query="UPDATE users SET account_status=1 WHERE email='$email'";
    return mysqli_query($db,$query);
}
